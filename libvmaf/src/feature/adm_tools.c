...
float adm_cm_s(const adm_dwt_band_t_s *src,const adm_dwt_band_t_s *csf_f,const float *obj, const adm_dwt_band_t_s *csf_a, int w, int h, int src_stride, int flt_stride, int csf_a_stride, double border_factor, int scale)
{
	...
	{
        obj_ = obj[0]/255;
		xh = src->band_h[0] * rfactor[0]*(1-p+p*obj_);
		xv = src->band_v[0] * rfactor[1]*(1-p+p*obj_);
		xd = src->band_d[0] * rfactor[2]*(1-p+p*obj_);
	...
	if (top <= 0) {
		for (j = start_col; j < end_col; ++j) {
            obj_ = 1-p+p*obj[j]/255;
			xh = src->band_h[j] * rfactor[0]*(1-p+p*obj_);
			xv = src->band_v[j] * rfactor[1]*(1-p+p*obj_);
			xd = src->band_d[j] * rfactor[2]*(1-p+p*obj_);
	...
	if ((top <= 0) && (right > (w - 1)))
	{
        obj_ = 1-p+p*obj[w-1]/255;
		xh = src->band_h[w - 1] * rfactor[0]*(1-p+p*obj_);
		xv = src->band_v[w - 1] * rfactor[1]*(1-p+p*obj_);
		xd = src->band_d[w - 1] * rfactor[2]*(1-p+p*obj_);
	...
	if ((left > 0) && (right <= (w - 1))) /* Completely within frame */
	{
		for (i = start_row; i < end_row; ++i) {
			...
			accum_inner_d = 0;
		for (j = start_col; j < end_col; ++j) {
                obj_ = 1-p+p*obj[i * src_px_stride + j]/255;
				xh = src->band_h[i * src_px_stride + j] * rfactor[0]*(1-p+p*obj_);
				xv = src->band_v[i * src_px_stride + j] * rfactor[1]*(1-p+p*obj_);
				xd = src->band_d[i * src_px_stride + j] * rfactor[2]*(1-p+p*obj_);
	...
	else if ((left <= 0) && (right <= (w - 1))) /* Right border within frame, left outside */
	{
		for (i = start_row; i < end_row; ++i) {
			...
			/* j = 0 */
            obj_ = 1-p+p*obj[i * src_px_stride]/255;
			xh = src->band_h[i * src_px_stride] * rfactor[0]*(1-p+p*obj_);
			xv = src->band_v[i * src_px_stride] * rfactor[1]*(1-p+p*obj_);
			xd = src->band_d[i * src_px_stride] * rfactor[2]*(1-p+p*obj_);
	...
	else if ((left > 0) && (right > (w - 1))) /* Left border within frame, right outside */
	{
		for (i = start_row; i < end_row; ++i) {
			...
			/* j = w-1 */
            obj_ = 1-p+p*obj[i * src_px_stride+w-1]/255;
			xh = src->band_h[i * src_px_stride + w - 1] * rfactor[0]*(1-p+p*obj_);
			xv = src->band_v[i * src_px_stride + w - 1] * rfactor[1]*(1-p+p*obj_);
			xd = src->band_d[i * src_px_stride + w - 1] * rfactor[2]*(1-p+p*obj_);
	...
	{
        obj_ = 1-p+p*obj[(h - 1) * src_px_stride]/255;
		xh = src->band_h[(h - 1) * src_px_stride] * rfactor[0]*(1-p+p*obj_);
		xv = src->band_v[(h - 1) * src_px_stride] * rfactor[1]*(1-p+p*obj_);
		xd = src->band_d[(h - 1) * src_px_stride] * rfactor[2]*(1-p+p*obj_);
	...
	if (bottom > (h - 1)) {
		for (j = start_col; j < end_col; ++j) {
            obj_ = 1-p+p*obj[(h - 1) * src_px_stride + j]/255;
			xh = src->band_h[(h - 1) * src_px_stride + j] * rfactor[0]*(1-p+p*obj_);
			xv = src->band_v[(h - 1) * src_px_stride + j] * rfactor[1]*(1-p+p*obj_);
			xd = src->band_d[(h - 1) * src_px_stride + j] * rfactor[2]*(1-p+p*obj_);
	...
	{
        obj_ = 1-p+p*obj[(h - 1) * src_px_stride + w - 1]/255;
		xh = src->band_h[(h - 1) * src_px_stride + w - 1] * rfactor[0]*(1-p+p*obj_);
		xv = src->band_v[(h - 1) * src_px_stride + w - 1] * rfactor[1]*(1-p+p*obj_);
		xd = src->band_d[(h - 1) * src_px_stride + w - 1] * rfactor[2]*(1-p+p*obj_);
