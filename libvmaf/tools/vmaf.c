...
static int validate_videos(video_input *vid1,video_input *vid2,video_input *vid3)
{
    int err_cnt = 0;

    video_input_info info1, info2,info3;
    video_input_get_info(vid1, &info1);
    video_input_get_info(vid2, &info2);
    video_input_get_info(vid3, &info3);

    if ((info1.frame_w != info2.frame_w) || (info1.frame_h != info2.frame_h)||(info2.frame_h != info3.frame_h)||(info2.frame_w != info2.frame_w)) {
        fprintf(stderr, "dimensions do not match: %dx%d, %dx%d ,%dx%d\n",
                info1.frame_w, info1.frame_h, info2.frame_w, info2.frame_h,info3.frame_w, info3.frame_h);
        err_cnt++;
    }

    if (info1.pixel_fmt != info2.pixel_fmt||info2.pixel_fmt != info3.pixel_fmt) {
        fprintf(stderr, "pixel formats do not match: %d, %d, %d\n",
                info1.pixel_fmt, info2.pixel_fmt,info3.pixel_fmt);
        err_cnt++;
    }

    if (!pix_fmt_map(info1.pixel_fmt) || !pix_fmt_map(info2.pixel_fmt)||!pix_fmt_map(info3.pixel_fmt) ) {
        fprintf(stderr, "unsupported pixel format: %d\n", info1.pixel_fmt);
        err_cnt++;
    }

    if (info1.depth != info2.depth||info2.depth != info3.depth) {
        fprintf(stderr, "bitdepths do not match: %d, %d ,%d\n",
                info1.depth, info2.depth,info3.depth);
        err_cnt++;
    }
...
int main(int argc, char *argv[])
{
    ...
    FILE *file_obj = fopen(c.path_obj, "rb");
    if (!file_obj) {
        fprintf(stderr, "could not open file: %s\n", c.path_obj);
        return -1;
    }
    ...
    video_input vid_obj;
    if (c.use_yuv) {
        err = raw_input_open(&vid_obj, file_obj,
                             c.width, c.height, c.pix_fmt, c.bitdepth);
    } else {
        err = video_input_open(&vid_obj, file_obj);
    }
    if (err) {
        fprintf(stderr, "problem with reference file: %s\n", c.path_obj);
        return -1;
    ...
    err = validate_videos(&vid_ref, &vid_dist,&vid_obj);
    if (err) {
    ...
    for (picture_index = 0 ;; picture_index++) {
        ...
        err = vmaf_read_pictures(vmaf, &pic_ref, &pic_dist, &pic_obj,picture_index);
        if (err) {
