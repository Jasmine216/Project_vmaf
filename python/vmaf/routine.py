...
def read_dataset(dataset, **kwargs):
    ...
    assert hasattr(dataset, 'obj_videos')
    ...
    for obj_video in obj_videos:
        obj_dict[obj_video['content_id']] = obj_video
    ...
    for dis_video in dis_videos:
        ...
        obj_video = obj_dict[obj_video['content_id']]

        ref_path = ref_video['path']
        obj_path = obj_video['path']

        ref_yuv_fmt_ = yuv_fmt if yuv_fmt is not None else ref_dict[dis_video['content_id']]['yuv_fmt']
        obj_yuv_fmt_ = obj_video['yuv_fmt'] if 'yuv_fmt' in obj_video else ref_yuv_fmt_
...
def run_test_on_dataset(test_dataset, runner_class, ax,
    ...
    for result in results:
        ...
        list_aggregate_score = list(map(lambda key: result[key], list_score_key))
        ...
        row = [result.asset.dataset,
               ...
               result.asset.asset_dict['dis_height'],
               result.asset.groundtruth
               ]
